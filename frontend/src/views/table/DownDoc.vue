            ...
            <el-table :data="table_list" class="auto_table" height="100%" v-loading="tableLoading" border
                ...
                <el-table-column align="center" label="操作" width="120" fixed="right">
                    <template slot-scope="scope">
                        <router-link :to="'DocDetails/'+scope.row.id">
            ...
            return{
                ...
                pickerOptions1: {
                    ...
                    disabledDate(date) { //disabledDate 文档上：设置禁用状态，参数为当前日期，要求返回 Boolean
                        return date.getTime() >= Date.now() || date.getTime() <= Date.now() - 3600 * 1000 * 24 * 31;
                    }
                },
                pickerOptions2: {
                    disabledDate(date) { //disabledDate 文档上：设置禁用状态，参数为当前日期，要求返回 Boolean
        ...
        methods:{
            ...
            extoexl(t){
                ...
                if(startTime > endTime) {
                    ...
                    window.location.href = "https://sai.bbxxjs.com/extoexl?token=" + token + "&t=" + t + "&st="+startTime+"&et="+endTime;
                }
            },
            formatDate(time) {
                if (time) {
